...
    		fileName = open.FileName;  
		...
		void getId(){
			try{
				SQLDataBase.conOpen();
				SqlCommand get = new SqlCommand("SELECT * FROM Company ORDER BY _id DESC",SQLDataBase.connection);
				int i;
				...
				textBox2.Focus();
				textBoxName.Text = "CMP-"+new_id.ToString();
			}catch(Exception){
				SQLDataBase.conClose();
			}
		}
		...
		void Button1Click(object sender, EventArgs e)
		{
			try{
				SQLDataBase.conOpen();
				SqlCommand get = new SqlCommand("SELECT * FROM Company where  CompanyId = '"+textBoxName.Text+"' ORDER BY _id DESC",SQLDataBase.connection);
				SqlDataReader rdr = get.ExecuteReader();
				if(rdr.Read()){
		...
		{
			try{
				string query = "DELETE from Company where _id=@_id";  
		...
		{
			try{
			string query = "INSERT INTO Company (_id, CompanyId, CompanyName, CompanyPhoneNumber, CompanyMobileNumber, CompanyCity, CompanyCountry, CompanyAddress, CompanyOwner, CompanyImage) VALUES (@_id, @CustomersId, @CustomersName, @CustomersPhoneNumber,@CustomersMobileNumber,@CustomersCity,@CustomersCountry,@CustomersAddress,@Remarks,@CustomersImage)";  
          	SqlCommand cmd = new SqlCommand(query, SQLDataBase.connection);  
          	cmd.Parameters.AddWithValue("@_id", new_id);  
		...
		{
			try{
			string query = "UPDATE Company SET CompanyName=@CustomersName, CompanyPhoneNumber=@CustomersPhoneNumber,CompanyMobileNumber=@CustomersMobileNumber,CompanyCity=@CustomersCity,CompanyCountry=@CustomersCountry,CompanyAddress=@CustomersAddress,CompanyOwner=@Remarks,CompanyImage=@CustomersImage where CompanyId=@CustomersId";  
          	SqlCommand cmd = new SqlCommand(query, SQLDataBase.connection);  
          	//cmd.Parameters.AddWithValue("@_id", new_id);  
          	cmd.Parameters.AddWithValue("@CustomersId", textBoxName.Text);    
          	cmd.Parameters.AddWithValue("@CustomersName", textBox2.Text);  
