...
    		fileName = open.FileName;  
		...
		{
			try{
			string query = "INSERT INTO Brands (_id, BrandName, BrandId, BrandImage) VALUES ( @_id, @BrandName, @BrandId, @BrandImage)";  
          	SqlCommand cmd = new SqlCommand(query, SQLDataBase.connection);  
          	cmd.Parameters.AddWithValue("@_id", new_id);  
          	cmd.Parameters.AddWithValue("@BrandName", textBoxName.Text);    
          	cmd.Parameters.AddWithValue("@BrandId", textBox1.Text);  
		...
		void getId(){
			try{
				SQLDataBase.conOpen();
				SqlCommand get = new SqlCommand("SELECT * FROM Brands ORDER BY _id DESC",SQLDataBase.connection);
				int i;
				...
				SQLDataBase.conClose();
				textBoxName.Focus();
				textBox1.Text = "BRD-"+new_id.ToString();
			}catch(Exception){
				SQLDataBase.conClose();
			}
		}
		...
		void Button1Click(object sender, EventArgs e)
		{
			try{
				SQLDataBase.conOpen();
				SqlCommand get = new SqlCommand("SELECT * FROM Brands where  BrandId = '"+textBox1.Text+"' ORDER BY _id DESC",SQLDataBase.connection);
				SqlDataReader rdr = get.ExecuteReader();
				if(rdr.Read()){
		...
		{
			try{
				string query = "DELETE from Brands where _id=@_id";  
          		SqlCommand cmd = new SqlCommand(query, SQLDataBase.connection);  
          		cmd.Parameters.AddWithValue("@_id", new_id);   
            	SQLDataBase.conOpen();
            	cmd.ExecuteScalar();  
		...
		{
			try{
			string query = "UPDATE Brands SET BrandName=@BrandName, BrandImage=@BrandImage WHERE _id=_id";  
          	SqlCommand cmd = new SqlCommand(query, SQLDataBase.connection);  
          	cmd.Parameters.AddWithValue("@BrandName", textBoxName.Text);    
          	cmd.Parameters.AddWithValue("@_id", new_id);  
			string img1= Utilites.ImageToBase64(pictureBox2.Image);
